...
public class SqlQueries {
    public static boolean userDatatableCreated(Connection connection) {
        try (PreparedStatement statement = connection.prepareStatement("SHOW TABLES LIKE 'userData';")){
    ...
    public static void createUserDataTables(Connection connection) {
        try (PreparedStatement statement = connection.prepareStatement("CREATE TABLE IF NOT EXISTS `userData` (`ID` INT(11) NOT NULL AUTO_INCREMENT,`UUID` VARCHAR(50) NULL DEFAULT '' COLLATE 'utf8mb4_general_ci',`Tag` VARCHAR(100) NULL DEFAULT '' COLLATE 'utf8mb4_general_ci',PRIMARY KEY (`ID`) USING BTREE)")) {
    ...
    public static void createTagsTables(Connection connection) {
        try (PreparedStatement statement = connection.prepareStatement("CREATE TABLE IF NOT EXISTS `tags` (`name` VARCHAR(100) NULL DEFAULT NULL COLLATE 'utf8mb4_general_ci',`inGameTag` VARCHAR(100) NULL DEFAULT NULL COLLATE 'utf8mb4_general_ci',`permission` VARCHAR(200) NULL DEFAULT NULL COLLATE 'utf8mb4_general_ci');")) {
    ...
    public static void createUserDataSQLiteTable(Connection connection) {
        try (PreparedStatement statement = connection.prepareStatement("CREATE TABLE IF NOT EXISTS 'userData' ('ID' INTEGER, 'UUID' TEXT, 'Tag' TEXT, PRIMARY KEY('ID' AUTOINCREMENT));")) {
    ...
    public static void createTagsDataSQLiteTable(Connection connection) {
        try (PreparedStatement statement = connection.prepareStatement("CREATE TABLE IF NOT EXISTS 'tags' ('name' TEXT, 'inGameTag' TEXT, 'permission' TEXT);")) {
    ...
    public static void insertData(Connection connection, UUID uuid, String tag) {
        try (PreparedStatement statement = connection.prepareStatement("INSERT INTO `userData` (`ID`, `UUID`, `Tag`) VALUES (null, ?, ?);")) {
    ...
    public static boolean playerInDB(Connection connection, UUID uuid) {
        try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM userData WHERE UUID=?")) {
            statement.setString(1, uuid.toString());
            ResultSet result = statement.executeQuery();
            boolean rb = result.next();
    ...
    public static void updateData(Connection connection, UUID uuid, String tag) {
        try (PreparedStatement statement = connection.prepareStatement("UPDATE `userData` SET `Tag` = ? WHERE UUID=?")) {
    ...
    public static String getTag(Connection connection, UUID uuid) {
        try (PreparedStatement statement = connection.prepareStatement("SELECT Tag From `userData` WHERE UUID=?")) {
    ...
    public static String getTagInGameTag(Connection connection, String tag) {
        try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM `tags` WHERE (name=?)")) {
    ...
    public static void createTag(Connection connection, String name, String inGameTag, String permission) {
        try (PreparedStatement statement = connection.prepareStatement("INSERT INTO `tags` (`name`, `inGameTag`, `permission`) VALUES (?, ?, ?)")) {
    ...
    public static void removeTag(Connection connection, String tag) {
        try (PreparedStatement statement = connection.prepareStatement("DELETE FROM tags WHERE (name=?)")) {
            statement.setString(1, tag);
            statement.executeUpdate();
        } catch (SQLException e) {
            throw new RuntimeException(e);
